...
class PlumbingPermitController extends Controller
{
	/**
	...
	{
		$model=new PlumbingPermit;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['PlumbingPermit']))
		{
			$model->attributes=$_POST['PlumbingPermit'];
			$model->app_status = PermitAppStatusEnum::PENDING;
			$model->request_date = new CDbExpression('NOW()');
			if($model->save()){
				Yii::app()->user->setFlash('plumbingPermitSuccess','Permit Application Successful.');
	...
	public function actionUpdate($id=null, $permit_num=null)
	{
		...
		if(isset($_POST['PlumbingPermit']))
		{
			...
			if($model->save()){
				Yii::app()->user->setFlash('plumbingPermitUpdateSuccess','Permit Successfully Updated.');
				$this->redirect(array('view','id'=>$model->id));
			}
		}
	...
	public function actionIndex()
	{
		...
		if($user_type == 0){// load only permits requested/applied for by citizen user
			...
			$dataProvider=new CActiveDataProvider('PlumbingPermit', array(
					'criteria'=>array(
							'condition'=>$condition,
			...
			$dataProvider=new CActiveDataProvider('PlumbingPermit');
			$this->render('index',array(
					'dataProvider'=>$dataProvider,
			));
		}
	...
	{
		$model=new PlumbingPermit('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['PlumbingPermit']))
			$model->attributes=$_GET['PlumbingPermit'];
	 ...
	 * @return PlumbingPermit the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=PlumbingPermit::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	{
		$model=PlumbingPermit::model()->findByAttributes(array('permit_num'=>$permit_num,));
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	public function loadModelPending($id=null,$permit_num=null)
	{
		$model=null;
		if (isset($id)){
			$model=PlumbingPermit::model()->findByAttributes(array('id'=>$id,'app_status'=>PermitAppStatusEnum::PENDING));
		} else if ( isset($permit_num)){
			$model=PlumbingPermit::model()->findByAttributes(array('permit_num'=>$permit_num,'app_status'=>PermitAppStatusEnum::PENDING));
		}
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	 ...
	 * @param PlumbingPermit $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='plumbing-permit-form')
		{
	...
	public function actionGeneratepdf($var){
		$model = PlumbingPermit::model()->findByPk($var);
		$pdf = Yii::app()->ePdf->mpdf();
		$pdf->WriteHTML($this->renderPartial('plumbingpermit', array('model'=>$model),true));
		$pdf->Output();
	}
