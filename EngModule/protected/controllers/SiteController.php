...
class SiteController extends Controller
{
	 ...
	 * Displays the login page
	 */
	public function actionCitizenLogin()
	{
		if(Yii::app()->user->isGuest){
			$model=new CitizenLoginForm;
			...
			// collect user input data
			if(isset($_POST['CitizenLoginForm']))
			{
				$model->attributes=$_POST['CitizenLoginForm'];
				// validate user input and redirect to the previous page if valid
				if($model->validate() && $model->login())
					$this->redirect(Yii::app()->user->returnUrl);
			}
			// display the login form
			$this->render('citizenLogin',array('model'=>$model));
		}else{
			$this->render('index');
		}
	}
	...
	public function actionExcel(){
		...
		$r->load(array(
				array(
						'id' => 'ong',
						'data' => array(
								'name' => 'UNIVERSIDAD aaaa NACIONAL'
						)
				),
				array(
						'id'=>'estu',
