...
class LoggableBehavior extends CActiveRecordBehavior{
	...
	public function auditAttributes($newattributes, $oldattributes = array()){
		...
		foreach ($newattributes as $name => $value) {
			...
			if ($value != $old) {
				$this->leaveTrail($this->getOwner()->getIsNewRecord() ? 'SAVE' : 'UPDATE', $this->getOwner()->getAttributeLabel($name), $value, $old);
			}
		}
	}
	...
	public function leaveTrail($action, $name = null, $value = null, $old_value = null){
		...
		$log->stamp		= new CDbExpression('NOW()');//$this->storeTimestamp ? time() : date($this->dateFormat); // If we are storing a timestamp lets get one else lets get the date
